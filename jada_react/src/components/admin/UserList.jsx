...
function UserList() {
   ...
   //   ----------------------------------
   // 초기 전체 회원 목록을 저장할 state 추가
   const [allUsers, setAllUsers] = useState([]);

   // 초기 데이터 로딩 시 전체 회원 목록을 설정
   useEffect(() => {
      const fetchAllUsers = async () => {
         try {
            const response = await axios.get(`${API_BASE_URL}/api/userinfo/allusers`);
   ...
   const handleSearch = () => {
      changeLoadingState();
      if (!searchTerm) {
         // 검색어가 없으면 전체 회원 목록을 사용
         setUsers(allUsers);
      } else {
         // 검색어가 있으면 임시로 저장된 전체 회원 목록에서 검색
         ...
         if (userInput === `삭제${user_id}`) {
            ...
            try {
               ...
               if (response.ok) {
                  ...
                  swal('오류', '회원삭제에 실패하였습니다. 다시 확인해주세요.', 'error');
               }
            } catch (error) {
               console.error('회원삭제 중 오류가 발생했습니다.', error);
            }
         } else {
            swal('오류', '삭제할 회원정보가 없습니다. 다시 확인해주세요.', 'error');
         }
      } else {
   ...
   const handleUserUpdate = async () => {
      if (selectedUser) {
         ...
         try {
            const response = await fetch(`${API_BASE_URL}/api/userinfo/admineditUser`, {
               method: 'POST',
               headers: {
                  'Content-Type': 'application/json',
               },
            ...
            if (response.ok) {
               ...
               swal('오류', '회원수정에 실패했습니다. 다시 확인해주세요.', 'error');
            }
         } catch (error) {
                     ...
                     {editMode && (
                        <>
                           <div className={style.update_btn_box}>
                              <button className={`${style.update_btn_on} ${style.save_btn}`} onClick={handleUserUpdate}>
                                 저장
                              </button>
                              <button

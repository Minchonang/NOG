   ...
   const handleEdit = async (e) => {
      ...
      if (newPhone !== null && newPhone) {
         ...
         if (!phoneRegex.test(newPhone)) {
            swal('오류', '휴대폰 번호 형식이 맞지 않습니다. \n예시: 01000000000', 'error');
            return;
         }
      ...
      if (newHouseNum !== null && newHouseNum) {
         ...
         if (!numberRegex.test(parseInt(newHouseNum))) {
            swal('오류', '가구원 수는 숫자만 입력해야 합니다.', 'error');
            return;
         }
      ...
      try {
         ...
         if (response.ok) {
            ...
            swal('성공', '회원 정보가 수정되었습니다.', 'success').then(() => {
               window.location.href = '/edit_userinfo';
            });
         } else {
            console.log('회원 정보 수정 실패');
            swal('오류', '회원 정보 수정에 실패하였습니다. 다시 확인해보세요.', 'error');
         }
      } catch (error) {
         console.error('서버 통신 오류', error);
      }
   ...
   const serialCheck = async () => {
      ...
      try {
         ...
         if (response.ok) {
            // // 서버 응답이 성공인 경우
            const result = await response.json();
            swal('성공', `${result.message}`, 'success');
            // 추출된 데이터 사용
            // setEditSerialNum(result.serialNum);
         } else {
            console.log('시리얼 번호 조회 실패');
            const result = await response.json();
            swal('오류', `${result.error}`, 'error');
         }
      } catch (error) {
         console.error('서버 통신 오류', error);
      }
   };
   // console.log(editSerialNum);
   return (
      <>
         {isLoading ? (
            <LoadingNog />
               ...
               <div className={common.background}>
                  ...
                  <div className={style.main_area}>
                     ...
                     <div className={style.info_main}>
                        <div className={style.info_main_admin_detail}>
                        ...
                        <div className={style.info_main_detail}>
                           {editEmail ? (
                              <>
                                 ...
                                 <button
                                    onClick={newEmail ? openModal : () => swal('', '변경할 이메일을 적어주십시오.')}
                                 >
                                    인증
                                 </button>
                              </>
                        ...
                        <div className={style.info_main_detail}>
                           <div className={style.serialNumInput_area}>
                              {editSerialNum ? (
                                 ...
                                 <>{userSerialNum ? <div>{userSerialNum}</div> : <div>등록된 기기가 없습니다.</div>}</>
                              )}
                              {!editSerialNum ? (
                     ...
                     <div className={style.info_sub}>
                        ...
                        <div className={style.info_sub_detail}>
                           <div className={style.addressInput_area}>
                              {editAddress ? (
                                 <div className={style.addressInput_area}>
                                    <div className={style.address_first_second_area}>
                                       <select

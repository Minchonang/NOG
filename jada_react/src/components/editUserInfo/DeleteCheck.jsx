...
function DeleteCheck() {
   ...
   const serverlink = async () => {
      ...
      try {
         ...
         if (response.ok) {
            ...
            swal('오류', '회원정보 조회에 실패했습니다. 다시 확인해주세요.', 'error');
         }
      } catch (error) {
   ...
   const handleDeletion = async () => {
      if (!isCheck) {
         swal('', '안내 사항 내용에 동의를 해주세요.');
      } else {
         if (!deletionReasonOption) {
            swal('', '회원탈퇴 사유를 입력해주세요.');
         } else {
            // user_id를 가져오기
            const user_id = sessionStorage.getItem('user_id');
            ...
            try {
               ...
               if (response.ok) {
                  swal('성공', '회원탈퇴가 완료되었습니다.', 'success').then(() => {
                     window.location.href = '/';
                  });
               } else {
                  // console.log(user_id);
                  swal('오류', '회원탈퇴에 실패했습니다. 다시 확인해주세요.', 'error');
               }
            } catch (error) {
         ...
         <div className={common.background}>
            ...
            <div className={style.main_area}>
               ...
               <div className={style.reason_area}>
                  ...
                  <div className={style.reason_area_detail}>
                     ...
                     >
                        ...
                        <option value="서비스 이용 빈도 감소">서비스 이용 빈도 감소</option>
               ...
               <div className={style.info_sub}>
                  <div className={style.info_sub_title}>회원정보/개인형 서비스 이용 기록 삭제</div>
                  <div className={style.info_sub_detail}>
                     <div>회원 정보 및 개인형 서비스 이용기록은 모두 삭제되며, 삭제된 데이터는 복구되지 않습니다.</div>
                     <div className={style.checkbox_area}>
                        ...
                        />
                        <label htmlFor="confirm_msg">위 내용을 모두 확인하였으며, 이에 동의합니다.</label>
                     </div>
                  </div>
               </div>
